...
# ==================== LOGGING / STRICT MODE ====================
# Padanan `set -euo pipefail` untuk Python: fail-fast + log error yang jelas.
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(name)s: %(message)s",
)
logger = logging.getLogger("fachio")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.warning(f"Gagal hapus {file_path}: {e}")
                return False

# ==================== UPLOAD ====================
async def upload_to_filebin(file_path: str, bin_name: str) -> str:
    """Upload ke Filebin (POST + header bin/filename + User-Agent). bin_name unik per file (privasi lebih baik)."""
    try:
        import httpx
        ...
        logger.error(f"Filebin gagal (status {response.status_code}): {response.text[:200]}")
        return None
    except Exception as e:
...
async def upload_to_gofile(file_path: str) -> str:
    """Upload ke Gofile via endpoint global terbaru (guest). Tiap file dapat link unik."""
    try:
        import httpx
        async with httpx.AsyncClient(timeout=300) as client:
            ...
            logger.error(f"Gofile upload failed: {data.get('message', 'Unknown error')}")
        return None
    except Exception as e:
...
async def queue_worker(user_id: str, app):
    ...
    while True:
        ...
        try:
            ...
            logger.error(f"Queue worker error (user {user_id}): {e}")
        finally:
            queue.task_done()
        ...
        with yt_dlp.YoutubeDL(opts) as ydl:
            try:
                ...
                if "not available" in error_msg or "no format" in error_msg:
                    logger.warning(f"Format fallback triggered: {e}")
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    """Tangkap semua exception di handler agar bot tidak diam-diam mati."""
    logger.error("Exception saat memproses update:", exc_info=context.error)

# ==================== MAIN ====================
def main():
    if TOKEN == "YOUR_BOT_TOKEN_HERE" or not TOKEN:
        logger.error("❌ BOT_TOKEN belum diisi! Jalankan setup.sh atau set env BOT_TOKEN.")
        sys.exit(1)
    ...
    logger.info(f"🤖 {BRAND_FULL} berjalan...")
    app.run_polling()
